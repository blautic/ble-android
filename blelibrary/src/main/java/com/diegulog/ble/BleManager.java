...
public class BleManager {
    ...
    private @NotNull final BleManagerCallback BleManagerCallback;
    private @NotNull final Map<String, BlePeripheral> connectedPeripherals = new ConcurrentHashMap<>();
    private @NotNull final Map<String, BlePeripheral> unconnectedPeripherals = new ConcurrentHashMap<>();
    private @NotNull final Map<String, BlePeripheral> scannedPeripherals = new ConcurrentHashMap<>();
    private @NotNull final List<String> reconnectPeripheralAddresses = new ArrayList<>();
    private @NotNull final Map<String, BlePeripheralCallback> reconnectCallbacks = new ConcurrentHashMap<>();
    ...
    private void sendScanResult(@NotNull final ScanResult result) {
        callBackHandler.post(new Runnable() {
            ...
            public void run() {
                if (isScanning()) {
                    final BlePeripheral peripheral = getPeripheral(result.getDevice().getAddress());
                    peripheral.setDevice(result.getDevice());
                    BleManagerCallback.onDiscoveredPeripheral(peripheral, result);
                }
            }
        });
    }
    ...
    private void sendScanFailed(@NotNull final ScanFailure scanFailure) {
        ...
        callBackHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                BleManagerCallback.onScanFailed(scanFailure);
            }
        });
    }
    ...
    private final ScanCallback autoConnectScanCallback = new ScanCallback() {
        ...
        public void onScanResult(final int callbackType, final ScanResult result) {
            synchronized (this) {
                ...
                final BlePeripheral peripheral = unconnectedPeripherals.get(deviceAddress);
                final BlePeripheralCallback callback = reconnectCallbacks.get(deviceAddress);
    ...
    protected final BlePeripheral.InternalCallback internalCallback = new BlePeripheral.InternalCallback() {
        @Override
        public void connected(@NotNull final BlePeripheral peripheral) {
            ...
            callBackHandler.post(new Runnable() {
                @Override
                public void run() {
                    BleManagerCallback.onConnectedPeripheral(peripheral);
                }
            });
        }

        @Override
        public void connectFailed(@NotNull final BlePeripheral peripheral, @NotNull final HciStatus status) {
            unconnectedPeripherals.remove(peripheral.getAddress());
            scannedPeripherals.remove((peripheral.getAddress()));
            ...
            if (nrRetries < MAX_CONNECTION_RETRIES && status != HciStatus.CONNECTION_FAILED_ESTABLISHMENT) {
                ...
                callBackHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        BleManagerCallback.onConnectionFailed(peripheral, status);
                    }
                });
            }
        }
        ...
        @Override
        public void disconnected(@NotNull final BlePeripheral peripheral, @NotNull final HciStatus status) {
            ...
            callBackHandler.post(new Runnable() {
                @Override
                public void run() {
                    BleManagerCallback.onDisconnectedPeripheral(peripheral, status);
                }
            });
        }

        @Override
        public @Nullable String getPincode(@NotNull final BlePeripheral device) {
     ...
     * @param BleManagerCallback the callback to call for updates
     * @param handler                  Handler to use for callbacks.
     */
    public BleManager(@NotNull final Context context, @NotNull final BleManagerCallback BleManagerCallback, @NotNull final Handler handler) {
        this.context = Objects.requireNonNull(context, "no valid context provided");
        this.BleManagerCallback = Objects.requireNonNull(BleManagerCallback, "no valid bluetoothCallback provided");
     ...
     */
    public void connectPeripheral(@NotNull final BlePeripheral peripheral, @NotNull final BlePeripheralCallback peripheralCallback) {
     ...
     */
    public void autoConnectPeripheral(@NotNull final BlePeripheral peripheral, @NotNull final BlePeripheralCallback peripheralCallback) {
        synchronized (connectLock) {
            Objects.requireNonNull(peripheral, NO_VALID_PERIPHERAL_PROVIDED);
            Objects.requireNonNull(peripheralCallback, NO_VALID_PERIPHERAL_CALLBACK_SPECIFIED);
    ...
    private void autoConnectPeripheralByScan(String peripheralAddress, BlePeripheralCallback peripheralCallback) {
     ...
     * @param peripheral the peripheral
     */
    public void cancelConnection(@NotNull final BlePeripheral peripheral) {
        Objects.requireNonNull(peripheral, NO_VALID_PERIPHERAL_PROVIDED);
        ...
        if (reconnectPeripheralAddresses.contains(peripheralAddress)) {
            ...
            callBackHandler.post(new Runnable() {
                @Override
                public void run() {
                    BleManagerCallback.onDisconnectedPeripheral(peripheral, HciStatus.SUCCESS);
                }
            });
     ...
     */
    public void autoConnectPeripheralsBatch(@NotNull final Map<BlePeripheral, BlePeripheralCallback> batch) {
        ...
        final Map<BlePeripheral, BlePeripheralCallback> uncachedPeripherals = new HashMap<>();
        for (BlePeripheral peripheral : batch.keySet()) {
            if (peripheral.isUncached()) {
        ...
        if (!uncachedPeripherals.isEmpty()) {
            for (BlePeripheral peripheral : uncachedPeripherals.keySet()) {
                final String peripheralAddress = peripheral.getAddress();
                reconnectPeripheralAddresses.add(peripheralAddress);
     ...
     */
    public @NotNull BlePeripheral getPeripheral(@NotNull final String peripheralAddress) {
        ...
        if (connectedPeripherals.containsKey(peripheralAddress)) {
            ...
            final BlePeripheral peripheral = new BlePeripheral(context, bluetoothAdapter.getRemoteDevice(peripheralAddress), internalCallback, new BlePeripheralCallback.NULL(), callBackHandler);
     ...
     */
    public @NotNull List<BlePeripheral> getConnectedPeripherals() {
    ...
    private void cancelAllConnectionsWhenBluetoothOff() {
        ...
        for (final BlePeripheral peripheral : connectedPeripherals.values()) {
            peripheral.disconnectWhenBluetoothOff();
        }
        connectedPeripherals.clear();

        // Call cancelConnection for unconnected peripherals
        for (final BlePeripheral peripheral : unconnectedPeripherals.values()) {
            peripheral.disconnectWhenBluetoothOff();
        }
        unconnectedPeripherals.clear();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
                ...
                callBackHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        BleManagerCallback.onBluetoothAdapterStateChanged(state);
                    }
                });
